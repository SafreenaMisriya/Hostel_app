...
class _AdminHostelFeeScreenState extends State<AdminHostelFeeScreen> {
  final CollectionReference _challanCollection = FirebaseFirestore.instance.collection('challan');
  ...
  void fetchAllChallansData() async {
    ...
    setState(() {
      ...
      challansList = challansSnapshot.docs
          .map((doc) => Challan.fromMap(doc.data() as Map<String, dynamic>)
        ..id = doc.id)
          .toList();
    });
  }

  Future<void> changeStatus(String challanId,) async {
    await FirebaseFirestore.instance.collection('challan').doc(challanId).update({'status': 'Paid'});
    setState(() {
      fetchAllChallansData();
    });
  }
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return GestureDetector(
                    onTap: () {
                      _showChallanDetailsDialog(context, challan);
                    },
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                _buildDetailItem('Parking Charges', challan.parkingCharges.toString()),
                _buildDetailItem('Electricity Fee', challan.electricityFee.toString()),
                _buildDetailItem('Water Charges', challan.waterCharges.toString()),
                _buildDetailItem('Room Charges', challan.roomCharges.toString()),
                _buildDetailItem('Total Amount', challan.beforeDueDate.toString()),
                _buildDetailItem('Issue Date', DateFormat('dd-MM-yyyy').format(challan.issueDate.toDate().toLocal())),
                _buildDetailItem('Due Date', DateFormat('dd-MM-yyyy').format(challan.dueDate.toDate().toLocal())),
              ],
            ),
          ),
          actions: <Widget>[
  ...
  Widget _buildHistoryCard(BuildContext context, String sr, String challan, String amount, String status) {
    return Card(
      elevation: 5,
  ...
  Widget _buildHistoryRow(String sr, String challan, String amount, String status) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        _buildRowItem(sr),
