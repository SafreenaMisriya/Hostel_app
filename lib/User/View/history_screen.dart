...
class _HistoryScreenState extends State<HistoryScreen> {
  ...
  void fetchChallansData() async {
    final QuerySnapshot challansSnapshot = await FirebaseFirestore.instance.collection('challan').get();
    setState(() {
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  if (challan['blockNo'] == userData?['block'] && challan['roomNo'] == userData?['room'] && challan['studentName'] == userData?['firstName']) {
                    return Column(
                      children: [
                        AppText(text:DateFormat('dd-MM-yyyy').format(challan['issueDate'].toDate().toLocal()),fontSize: 14,fontWeight: FontWeight.w500, ),
                        _buildHistoryCard(
                          context,
                          sr: (index + 1).toString(),
                          challan: challan,
  ...
  Widget _buildStatusColor(String status ,) {
    Color statusColor;
    if (status == 'Paid') {
